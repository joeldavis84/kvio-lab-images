...
cloudEnvironments.each { environName, environValues ->
  ...
  builders[podName] = {
    ...
    deployOpenShiftTemplate(containersWithProps: containers, openshift_namespace: 'kubevirt', podName: podName,
      ...
      ciPipeline(buildPrefix: 'kubevirt-image-builder', decorateBuild: decoratePRBuild(), archiveArtifacts: archives, timeout: 240, sendMetrics: false) {
        try {
          ...
          stage("${environName}-build-images") {
            executeInContainer(containerName: 'ansible-executor', containerScript: "sh -c env", stageVars: params, credentials: credentials)                                       
          }
